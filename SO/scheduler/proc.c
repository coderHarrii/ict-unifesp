 ...
 *				enqueue					     * 
 *===========================================================================*/
/* Responsavel por colocar um processo recem criado ou acordado em sua respectiva fila de processos*/
void enqueue(
 ...
 */
  ...
  /*
   * se alteramos o valor de prioridade do processo para a maior prioridade possível
   * teremos todos os processos em uma mesma fila de prioridades sempre, mantendo todos em round robin
   *
   * int q = rp->p_priority;	 		/* scheduling queue to use 
   */
  int q = rp->p_priority = 0;

  struct proc **rdy_head, **rdy_tail;

  /* verifica se o processo esta pronto para execução*/
  assert(proc_is_runnable(rp));

  /* testa se o processo pode ser executado dentro das filas de prioridade existentes */
  assert(q >= 0);

  /* obtem o ponteiro para a cabeça da fila de processos prontos na CPU do processo "rp" */
  rdy_head = get_cpu_var(rp->p_cpu, run_q_head);

  /* obtem o ponteiro para a cauda da fila de processos na CPU do processo "rp" */
  ...
  /* 
   * verifica se o id da cpu atual eh igual ao id da cpu do processo, 
   * garantindo o enfileiramento na CPU correta do processo
   */
  if (cpuid == rp->p_cpu) {
	  /*
	  ...
	  /*
	   * verifica se o processo que entrou na fila tem prioridade maior do que o processo em execucao no momento 
       * sera sempre falso nesse caso. No funcionamento padrao, o processo em execucao sofreria preempcao 
	   * se fosse preemptivel
	   */
...
{
  /*
   * a ideia aqui eh a mesma de enqueue, se alteramos o valor de prioridade 
   * do processo para a maior prioridade possível teremos todos os processos
   * carregados em uma mesma fila, ou seja, sempre em round robin
   * 
   * int q = rp->p_priority;	 		/* scheduling queue to use 
   */
  int q = rp->p_priority = 0;
  ...
  /* verifica se o ponteiro pra processo aponta pra um endereco valido */
  assert(proc_ptr_ok(rp));
  
  /* verifica se o processo eh executavel */
  assert(proc_is_runnable(rp));
  ...
  /* garante se o processo pertence a alguma fila valida */
  assert(q >= 0);

  /* obtem o ponteiro para a cabeça da fila de processos prontos na CPU do processo "rp" */
  rdy_head = get_cpu_var(rp->p_cpu, run_q_head);

  /* obtem o ponteiro para a cauda da fila de processos prontos na CPU do processo "rp" */
  rdy_tail = get_cpu_var(rp->p_cpu, run_q_tail);
 ...
 */
  /*
   * a ideia aqui eh a mesma de enqueue e enqueue_head, 
   * se alteramos o valor de prioridade do processo para 
   * a maior prioridade possível teremos todos os processos
   * carregados em uma mesma fila, ou seja, sempre em round robin
   * 
   * int q = rp->p_priority;	 		/* scheduling queue to use 
   */
 ...
 */
   ...
   * 
   * No caso de implementar a politica de escalonamento round robin, teremos apenas
   * uma unica fila de processos, de prioridade 0 
   */
  rdy_head = get_cpulocal_var(run_q_head);
  
  // verifica se o processo esta pronto pra execucao na fila, se nao estiver, printa a mensagem
  ...
  for (q=0; q < 1; q++) {	
	...
	/*
	* a ideia aqui eh a mesma de enqueue, enqueue_head e dequeue 
	* se alteramos o valor de prioridade do processo para 
	* a maior prioridade possível teremos todos os processos
	* carregados em uma mesma fila, ou seja, sempre em round robin
	*
	* ou seja, se o processo for encontrado a prioridade dele será 0
	*/
	rp->priority = 0;

	/* verifica se o processo esta pronto */
	assert(proc_is_runnable(rp));
	
	// registra qual processo sera cobrado pelo tempo de CPU usado pelo sistema
