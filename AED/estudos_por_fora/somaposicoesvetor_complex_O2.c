...
int somavetorcomponteiro(int Vetor[],const int n){//O(M)
    int soma=0;
    ...
    for(ponteiro=Vetor;ponteiro<FinaldoVetor;ponteiro++){ //ponteiro=vetor[0]|ponteiro=vetor[1]|ponteiro=vetor[2]|ponteiro!=vetor[3]=FinaldoVetor
        soma+=*ponteiro;//O(M)
    }
    return soma;//O(1)
}// Acaba que a complexidade aqui eh de O(N) * O(N) = O(N^2)
